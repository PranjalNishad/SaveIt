...
function buildArgs(url: string, outputPath: string, format: OutputFormat, platform: Platform): string[] {
    const args: string[] = [
        "--no-playlist",
        "--no-warnings",
        "--socket-timeout",
    ...
    if (format === "audio") {
        args.push("-x", "--audio-format", DOWNLOAD.AUDIO_FORMAT, "--audio-quality", DOWNLOAD.AUDIO_QUALITY, url);
        return args;
    }
...
async function runYtDlp(args: string[]): Promise<{ code: number | null; stdout: string; stderr: string }> {
...
export async function downloadMedia(url: string, format: OutputFormat, platform: Platform): Promise<DownloadResult> {
    ...
    try {
        ...
        if (code !== 0) {
            return { success: false, error: stderr || `yt-dlp exited with code ${code}` };
        }
